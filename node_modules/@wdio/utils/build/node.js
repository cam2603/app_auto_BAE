...
function getChromedriverCdnUrl() {
  return process.env.CHROMEDRIVER_CDNURL?.trim().replace(/\/+$/, "") || void 0;
}
var canAccess = (file) => {
  if (!file) {
    return false;
...
function redactCredentials(message) {
  return message.replace(/([a-zA-Z][\w+.-]{0,30}:\/\/)[^/\s?#]+@/g, "$1").replace(/([a-zA-Z][\w+.-]{0,30}:\/\/[^\s"'?#]*)\?[^\s"']*/g, "$1?[redacted]");
}
...
async function setupChromedriver(cacheDir, driverVersion) {
  ...
  if (!hasChromedriverInstalled) {
    ...
    if (await canDownload(chromedriverInstallOpts)) {
      ...
      log.warn(
        `Chromedriver v${buildId} don't exist, trying to find known good version...` + (cdnUrl ? ` (checked ${redactCredentials(cdnUrl)} from CHROMEDRIVER_CDNURL, a failed request is reported the same way as a missing version)` : "")
      );
...
var TRANSIENT_SPAWN_CODES = /* @__PURE__ */ new Set(["EBUSY", "EAGAIN", "EACCES"]);
var SPAWN_RETRY_ATTEMPTS = 5;
var SPAWN_RETRY_DELAY = 200;
async function spawnDriver(command, args, options) {
  let lastError;
  for (let attempt = 1; attempt <= SPAWN_RETRY_ATTEMPTS; attempt++) {
    try {
      return cp2.spawn(command, args, options);
    } catch (err) {
      lastError = err;
      const code = err.code;
      ...
      log2.warn(`Failed to spawn ${command} (${code}), retrying ${attempt}/${SPAWN_RETRY_ATTEMPTS} ...`);
