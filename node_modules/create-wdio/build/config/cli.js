...
async function setupTypeScript(parsedAnswers) {
  ...
  const defaultSpecInclude = (parsedAnswers.destSpecRootPath && path.relative(parsedAnswers.projectRootDir, parsedAnswers.destSpecRootPath) || parsedAnswers.specs && path.dirname(
    parsedAnswers.specs.split(/[\\/]/).filter((segment) => !segment.includes("*")).join(path.sep)
  ) || "test").replace(/\\/g, "/");
  const defaultWdioConfigInclude = (parsedAnswers.wdioConfigPath && path.relative(parsedAnswers.projectRootDir, parsedAnswers.wdioConfigPath) || "wdio.conf.ts").replace(/\\/g, "/");
