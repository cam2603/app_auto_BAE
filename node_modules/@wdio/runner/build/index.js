...
import { initializeWorkerService, initializePlugin as initializePlugin2, executeHooksWithArgs as executeHooksWithArgs2, TimingTracker } from "@wdio/utils";
import { ConfigParser } from "@wdio/config/node";
import { _setGlobal } from "@wdio/globals";
import { expect as expect2, setDefaultOptions, getDefaultOptions, wdioCustomMatchers as wdioCustomMatchers2, SnapshotService, SoftAssertionService } from "expect-webdriverio";
...
function isSerializedAsymmetricMatcher(arg) {
  return typeof arg === "object" && arg !== null && "$$typeof" in arg && typeof arg.$$typeof === "string" && Object.keys(SUPPORTED_ASYMMETRIC_MATCHER).includes(arg.$$typeof);
}
function isOneOfMatcher(matcherKey) {
...
function transformExpectArgs(arg) {
  ...
  const matcher = arg.inverse ? expect.not[inverseMatcherKey] : expect[matcherKey];
  if (!matcher) {
...
${ev.error ? ev.error.split("\n").slice(1).join("\n") : ""}`);
  ...
  async #handleExpectation(id, payload) {
    ...
    try {
      ...
      const refetchElement = async (element) => {
        return element.elementId ? await browser.$(element) : await browser.$(element.selector);
      };
      if (received) {
      ...
      if (isSome) {
        received = isSome ? some(received) : received;
      }
    ...
    if (runner === "local") {
      ...
      if (frameworkInstance.setupExpect) {
        const matchersShim = Object.defineProperty(
          ...
          {
            enumerable: false,
            value: () => Object.entries(wdioCustomMatchers2)[Symbol.iterator]()
          }
        );
        await frameworkInstance.setupExpect(expect2, matchersShim, getDefaultOptions);
      }
