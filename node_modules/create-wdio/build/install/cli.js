...
function buildDioxusBanner(driverProvider) {
  const lines = [
    "",
    "\u{1F980} Dioxus requires a couple of Rust-side additions before tests can run:",
    "",
  ...
  if (driverProvider === "External driver via wdio-dioxus-driver (Windows only)" /* External */) {
    lines.push(
  ...
  lines.push(
    ...
    "  \u{1F517} https://webdriver.io/docs/desktop-testing/dioxus"
  );
  return lines.join("\n");
}
...
var DesktopFrameworkChoice = /* @__PURE__ */ ((DesktopFrameworkChoice2) => {
  ...
  DesktopFrameworkChoice2["Dioxus"] = "Dioxus (https://dioxuslabs.com/)";
...
var DioxusDriverProviderChoice = /* @__PURE__ */ ((DioxusDriverProviderChoice2) => {
  DioxusDriverProviderChoice2["Embedded"] = "Embedded in-process driver (recommended \u2014 no external driver, all platforms)";
  DioxusDriverProviderChoice2["External"] = "External driver via wdio-dioxus-driver (Windows only)";
  return DioxusDriverProviderChoice2;
...
function getResolvedPurpose(answers) {
  ...
  switch (answers.desktopFramework) {
    case "Tauri (https://tauri.app/)" /* Tauri */:
      return "tauri";
    case "Dioxus (https://dioxuslabs.com/)" /* Dioxus */:
      return "dioxus";
...
var QUESTIONNAIRE = [{
  ...
  when: (
    /* istanbul ignore next */
    (answers) => getResolvedPurpose(answers) === "dioxus"
  )
}, {
  type: "input",
...
async function setupTypeScript(parsedAnswers) {
  ...
  const tsConfigDir = path2.dirname(parsedAnswers.tsConfigFilePath);
  const toPosix = (value) => value.replace(/\\/g, "/");
  const getRelativePath = (target) => {
    const relativePath = toPosix(
      path2.relative(tsConfigDir, path2.resolve(parsedAnswers.projectRootDir, target))
    );
    return relativePath || ".";
  };
  const toExtendsPath = (target) => {
    const relativePath = getRelativePath(target);
    if (relativePath === "." || relativePath.startsWith("./") || relativePath.startsWith("../") || path2.isAbsolute(relativePath)) {
      return relativePath;
    }
    return `./${relativePath}`;
  };
  ...
  const defaultSpecInclude = (parsedAnswers.destSpecRootPath && path2.relative(parsedAnswers.projectRootDir, parsedAnswers.destSpecRootPath) || parsedAnswers.specs && path2.dirname(
    parsedAnswers.specs.split(/[\\/]/).filter((segment) => !segment.includes("*")).join(path2.sep)
  ) || "test").replace(/\\/g, "/");
  const defaultWdioConfigInclude = (parsedAnswers.wdioConfigPath && path2.relative(parsedAnswers.projectRootDir, parsedAnswers.wdioConfigPath) || "wdio.conf.ts").replace(/\\/g, "/");
  const baseIncludes = preset === "svelte" ? ["src/**/*.d.ts", "src/**/*.ts", "src/**/*.js", "src/**/*.svelte"] : preset === "vue" ? ["src/**/*.ts", "src/**/*.d.ts", "src/**/*.tsx", "src/**/*.vue"] : [
...
function resolveTsConfigFilePath({
  ...
  const rootTsConfigPath = path3.resolve(projectRootDir, "tsconfig.json");
  const dedicatedTsConfigPath = path3.resolve(
    projectRootDir,
  ...
  const specPathWithoutGlobs = specs.split(path3.sep).filter((segment) => !segment.includes("*")).join(path3.sep);
  const candidate = path3.resolve(projectRootDir, path3.dirname(specPathWithoutGlobs), "tsconfig.json");
  return path3.resolve(candidate) === rootTsConfigPath ? dedicatedTsConfigPath : candidate;
}
