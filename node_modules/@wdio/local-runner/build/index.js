...
var GRACEFUL_SHUTDOWN_SIGNALS = ["SIGTERM", "SIGINT"];
var SIGNAL_EXIT_CODE_OFFSET = 128;
var UNKNOWN_SIGNAL_EXIT_CODE = 1;
...
var WorkerInstance = class extends EventEmitter {
   ...
   */
  _handleExit(exitCode, signal = null) {
    ...
    if (crashed) {
      log.error(
        `Runner ${cid} was terminated by ${signal} and did not report an exit code. The spec file(s) ${specs.join(", ")} are reported as failed with exit code ${resolvedExitCode}. This usually means the process ran out of memory or a native module crashed.`
      );
    } else {
