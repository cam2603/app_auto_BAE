...
function WebDriver(options, modifier, propertiesObject = {}) {
  ...
  const emitDialogListenerTransition = (previousCount) => {
  ...
  const addDialogListener = (listener, registered = listener) => {
  ...
  const removeDialogListener = (listener, registered) => {
    const previousCount = dialogListeners.length;
    let index = -1;
    for (let i = dialogListeners.length - 1; i >= 0; i--) {
      const entry2 = dialogListeners[i];
      if (registered ? entry2.registered === registered : entry2.listener === listener) {
        index = i;
        break;
      }
    }
...
function ensureFileURL(path2) {
  ...
  if (REG_EXP_WINDOWS_ABS_PATH.test(path2)) {
    const [drive, ...segments] = path2.replace(/\\/g, "/").split("/");
...
async function initializeLauncherService(config, caps) {
  ...
  const workerServiceLoadDecisions = /* @__PURE__ */ new Map();
  ...
  try {
    ...
    for (const [service, serviceConfig, serviceName] of services) {
      ...
      if (serviceName && (typeof servicePlugin.default !== "function" && typeof service !== "function" || workerServiceLoadDecisions.get(servicePlugin) === false) && !ignoredWorkerServices.includes(serviceName)) {
...
async function executeAsync(fn, retries, args = [], timeout = 2e4) {
  ...
  try {
    ...
    const fullTitle = typeof runnable?.fullTitle === "function" ? runnable.fullTitle() : void 0;
...
var testFrameworkFnWrapper = async function({ executeHooksWithArgs: executeHooksWithArgs2, executeAsync: executeAsync2 }, type, { specFn, specFnArgs }, { beforeFn, beforeFnArgs }, { afterFn, afterFnArgs }, cid, repeatTest = 0, hookName, timeout) {
  ...
  try {
    ...
    if (!(JSON.stringify(_err, Object.getOwnPropertyNames(_err)).includes("sync skip; aborting execution") || JSON.stringify(_err, Object.getOwnPropertyNames(_err)).includes("marked Pending"))) {
      const err = _err instanceof Error ? _err : new Error(
        typeof _err === "string" ? _err : `An unknown error occurred: ${JSON.stringify(_err)}`
      );
      if (err.stack) {
...
var TimingTracker = class {
  ...
  getTimings() {
    const result = {};
    if (this.timings.setupStart !== void 0 && this.timings.setupEnd !== void 0) {
      result.setup = (this.timings.setupEnd - this.timings.setupStart) / 1e3;
    }
    if (this.timings.executionStart !== void 0 && this.timings.executionEnd !== void 0) {
      result.execution = (this.timings.executionEnd - this.timings.executionStart) / 1e3;
    }
    if (this.timings.teardownStart !== void 0 && this.timings.teardownEnd !== void 0) {
      result.teardown = (this.timings.teardownEnd - this.timings.teardownStart) / 1e3;
    }
    if (this.timings.setupStart !== void 0 && this.timings.teardownEnd !== void 0) {
      result.total = (this.timings.teardownEnd - this.timings.setupStart) / 1e3;
    }
    return result;
  }
  /**
