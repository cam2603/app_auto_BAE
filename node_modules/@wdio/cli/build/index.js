...
var Launcher = class {
  ...
  async _startInstance(specs, caps, cid, rid, retries) {
    ...
    const profilingArgs = [];
    const enableCpuProf = Boolean(this._args.cpuProf || config3.cpuProf);
    const enableHeapProf = Boolean(this._args.heapProf || config3.heapProf);
    const profileOutputDir = config3.profileOutputDir || "./profiles";
    if (enableCpuProf || enableHeapProf) {
      try {
        await fs.mkdir(profileOutputDir, { recursive: true });
      } catch (err) {
        log3.warn(`Failed to create profiling output directory "${profileOutputDir}": ${err.message}`);
      }
      if (!this._hasLoggedProfiling) {
        const enabled = [
          enableCpuProf ? "CPU" : void 0,
          enableHeapProf ? "Heap" : void 0
        ].filter(Boolean).join(" + ");
        log3.info(`Profiling enabled (${enabled}). Output: ${profileOutputDir}`);
        this._hasLoggedProfiling = true;
      }
    }
    const hasExecArg = (argPrefix) => capExecArgs.some((arg) => arg === `--${argPrefix}` || arg.startsWith(`--${argPrefix}=`));
    if (enableCpuProf) {
  ...
  async _endHandler({ cid: rid, exitCode, specs, retries, signal }) {
    ...
    const config3 = this.configParser.getConfig();
    await runLauncherHook(config3.onWorkerEnd, rid, exitCode, specs, retries, signal).catch((error) => this._workerHookError(error));
    await runServiceHook(this._launcher, "onWorkerEnd", rid, exitCode, specs, retries, signal).catch((error) => this._workerHookError(error));
...
async function handler(argv) {
  ...
  const hasLocalTSConfig = await fs2.access(localTSConfigPath).then(() => true, () => false);
  if (hasLocalTSConfig) {
